-- ============================================
-- FIX: Get decision from the right place
-- The decision might be in ratings JSONB or decision column
-- ============================================
...
BEGIN
  FOREACH _app_id IN ARRAY p_application_ids LOOP
    ...
    INSERT INTO public.application_publications(application_id, org_id, published_by, payload, visibility, version)
    VALUES (_app_id, _org_id, auth.uid(), _payload, p_visibility, 1)
    ...
    -- Decrement spots - NOW _decision might come from ratings JSONB too
    IF p_acceptance_tag IS NOT NULL AND _program_id IS NOT NULL AND _decision IS NOT NULL THEN
