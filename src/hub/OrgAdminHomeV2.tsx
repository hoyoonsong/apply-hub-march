...
export default function OrgAdminHomeV2() {
  const { orgSlug } = useParams<{ orgSlug: string }>();
  const navigate = useNavigate();
  const location = useLocation();
  const { showCreateModal } = useOrgAdminModals();
  ...
  // Gate: ensure user is allowed and load programs
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-7xl mx-auto px-8 py-8">
            {/* Big Create Button - Google Docs style */}
            <div className="mb-8">
              <button
                onClick={showCreateModal}
                className="group relative w-full h-48 border-2 border-dashed border-gray-300 rounded-xl hover:border-indigo-400 hover:bg-indigo-50/50 transition-all duration-200 flex flex-col items-center justify-center gap-4"
              >
                <div className="w-16 h-16 rounded-full bg-indigo-600 group-hover:bg-indigo-700 flex items-center justify-center transition-colors shadow-lg">
                  <svg
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-gray-500">
                                    {searchTerm
                                      ...
                                      : "Create your first program using the button above to get started."}
                                  </p>
                                </div>
                              </div>
                            </td>
